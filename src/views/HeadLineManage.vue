<template>
    ...
    >
      ...
      >
        <n-grid :cols="24" :x-gap="24">
          <n-form-item-gi :span="12" label="名称" path="Title">
            <n-input v-model:value="HeadlienModel.Title" placeholder="Input" />
          </n-form-item-gi>
          <n-form-item-gi :span="12" label="链接" path="DescriptionURL">
            <n-input v-model:value="HeadlienModel.DescriptionURL" placeholder="Input" />
          </n-form-item-gi>
      ...
      <template #action>
        ...
        <n-button @click="(showAddAdmin = false)">取消</n-button>
      </template>
    </n-modal>
    <!-- 修改头条的模型窗口 -->
    <n-modal
    ...
    >
      ...
      <n-grid :cols="24" :x-gap="24">
          <n-form-item-gi :span="12" label="标题" path="Title">
            <n-input v-model:value="UpdateModel.Title" placeholder="Input" />
          </n-form-item-gi>
          <n-form-item-gi :span="12" label="链接" path="DescriptionURL">
            <n-input v-model:value="UpdateModel.DescriptionURL" placeholder="Input" />
          </n-form-item-gi>
  ...
  import { NButton, useMessage, NAvatar, NTag,useDialog,NImage } from "naive-ui";
  import {
    UserPageQuery,
    DelHeadline,
    AddHeadline,
  ...
  //初始化
  function Rest() {
    (HeadlienModel.CoverURL = ""), (HeadlienModel.DescriptionURL = "");
    (HeadlienModel.Title = "");
  }
  function Rest2() {
  ...
  const toUpdate = (row) => {
    ...
    UpdateModel.Title = row.title;
    UpdateModel.DescriptionURL = row.descriptionURL;
    UpdateModel.CoverURL = row.coverURL;
    temp.value=row.coverURL;
    adminId=row.id;
  };
    ...
    formRef.value?.validate((errors) => {
      if (!errors) {
        uploadadd(fileRef.value).then(() => {
          HeadlienModel.CoverURL = temp.value;
          AddHeadline(HeadlienModel).then((res) => {
            if (res.data.success == true) {
              loadingData(paginationReactive.pageIndex);
              message.success("头条添加成功");
              Rest();
            } else {
              message.success("头条添加失败:"+res.data.data);
            }
          });
        });
      } else {
  ...
  const submitUpdateCallback = (id) => {
    formRef.value?.validate((errors) => {
      if (!errors) {
        uploadadd(fileRef.value).then(() => {
          UpdateModel.CoverURL = temp.value;
          UpdateHeadline(id, UpdateModel).then((res) => {
            if (res.data.success == true) {
              loadingData(paginationReactive.pageIndex);
              message.success(res.data.message + res.data.data);
              Rest();
  ...
  let rules = {
    ...
    DescriptionURL: [
      ...
      { min: 10, max: 50, message: "描述长度在10-50个字符", trigger: "blur" },
    ],
